...
export const reportContent = (input: { contentType: 'post' | 'comment' | 'profile' | 'recommendation'; contentId: string; reason: string; parentId?: string }) => callFunction<typeof input, { ok: boolean; flagId: string; alreadyReported: boolean }>('reportContent', input);
